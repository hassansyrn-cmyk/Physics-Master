...
namespace PhysicsMaster.Gameplay {
    public sealed class DrawingController : MonoBehaviour {
        ...
        readonly List<GameObject> strokes = new();
        readonly List<Vector2> points = new();
        Camera worldCamera;
        LineRenderer activeLine;
        float usedInk, activeInk;
        Vector2 start;
        ...
        Vector2 WorldPoint() {
            Vector2 p = worldCamera.ScreenToWorldPoint(Input.mousePosition);
            Rect r = PhysicsMaster.Core.AppController.PlayableWorldRect;
            return new Vector2(Mathf.Clamp(p.x, r.xMin, r.xMax), Mathf.Clamp(p.y, r.yMin, r.yMax));
        }

        void Begin(Vector2 p) {
            if (RemainingInk <= .05f) return;
            Rect r = PhysicsMaster.Core.AppController.PlayableWorldRect;
            ...
            points.Add(p);
            activeInk = 0;
            activeLine = CreateLine("DrawingPreview");
        ...
        void Drag(Vector2 p) {
            ...
            float segment = Vector2.Distance(points[^1], p);
            if (usedInk + activeInk + segment > MaxInk) return;

            activeInk += segment;
            points.Add(p);
            activeLine.positionCount = points.Count;
            activeLine.SetPosition(points.Count - 1, p);
        }

        void SetPreview(IReadOnlyList<Vector2> pts) {
            points.Clear();
            for (int i = 0; i < pts.Count; i++) points.Add(pts[i]);
            activeLine.positionCount = points.Count;
            for (int i = 0; i < points.Count; i++) activeLine.SetPosition(i, points[i]);
            activeInk = Length(points);
        }
        ...
        void End(Vector2 p) {
            ...
            edge.points = clean.ToArray();
            edge.edgeRadius = .08f;

            var body = activeLine.gameObject.AddComponent<Rigidbody2D>();
            body.bodyType = RigidbodyType2D.Dynamic;
            body.simulated = false;
            body.mass = Mathf.Clamp(activeInk * .18f, .2f, 4f);

            if (Tool == DrawingTool.Pin) {
                body.bodyType = RigidbodyType2D.Static;
                activeLine.startColor = activeLine.endColor = PhysicsMaster.Core.Theme.Purple;
            }

            strokes.Add(activeLine.gameObject);
            usedInk += activeInk;
            activeLine = null;
            points.Clear();
        }

        LineRenderer CreateLine(string n) {
            var g = new GameObject(n);
            GameObject w = GameObject.Find("World");
            if (w != null) g.transform.SetParent(w.transform);

            var l = g.AddComponent<LineRenderer>();
            l.material = new Material(Shader.Find("Sprites/Default"));
            l.startColor = l.endColor = InkColor;
            l.startWidth = l.endWidth = .18f;
            l.numCapVertices = 6;
        ...
        static List<Vector2> Simplify(List<Vector2> src, float min) {
            var o = new List<Vector2> { src[0] };
            for (int i = 1; i < src.Count - 1; i++) if (Vector2.Distance(o[^1], src[i]) >= min) o.Add(src[i]);
            o.Add(src[^1]);
            return o;
        }

        static float Length(IReadOnlyList<Vector2> p) {
            float n = 0;
            for (int i = 1; i < p.Count; i++) n += Vector2.Distance(p[i - 1], p[i]);
            return n;
        }

        static Vector2[] CirclePoints(Vector2 c, float r) {
            int n = 28;
        ...
        float LengthFromLine(LineRenderer l) {
            float n = 0;
            for (int i = 1; i < l.positionCount; i++) n += Vector3.Distance(l.GetPosition(i - 1), l.GetPosition(i));
            return n;
        }
        ...
        public void StartSimulation() {
            ...
            foreach (var g in strokes) {
                var b = g ? g.GetComponent<Rigidbody2D>() : null;
                if (b && b.bodyType == RigidbodyType2D.Dynamic) b.simulated = true;
            }
            foreach (var b in FindObjectsByType<Rigidbody2D>(FindObjectsSortMode.None)) {
                if (b.gameObject.name.StartsWith("Dynamic")) b.simulated = true;
            }
        }
